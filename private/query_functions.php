<?php
  ...
  function insert_failed_login($failed){
    global $db;
    $sql = "INSERT INTO failed_logins ";
    $sql .= "(username, count, last_attempt) ";
    $sql .= "VALUES (";
    $sql .= "'" . db_escape($db, $failed['username']) . "',";
    $sql .= "'" . db_escape($db, $failed['count']) . "',";
    $sql .= "'" . db_escape($db, $failed['last_attempt']) .  "'";
    $sql .= ");";
    // For INSERT statements, $result is just true/false
    $result = db_query($db, $sql);
    if($result) {
  ...
  function validate_user($user, $errors=array()) {
    if (has_valid_pw($user['password_confirmation'], $user['password']) == false){
    ...
    if (!has_strong_password($user['password'])){
      $errors[] = "Password does not contain at least one of each: uppercase letter, lowercase letter, letter, symbol";
    }
  ...
  function insert_user($user) {
    ...
    $sql .= "(first_name, last_name, email, username, created_at, password) ";
    $sql .= "VALUES (";
