...
if(is_post_request() && request_is_same_domain()) {
  ...
  if (empty($errors)) {
    ...
    $users_result = find_users_by_username(u(h($username)));
    // No loop, only one result
    $user = db_fetch_assoc($users_result);
    if($user) {
      $remains = throttle_time($user['username']);
      if(password_verify($password, $user['password']) &&($remains <= 0)) {
        ...
        //echo "<br>remains: " .$remains;
        if($remains != 0){
          $errors[] = "Too many failed logins for this username " .$remains . " seconds remain to log in";
        }
        else{
